...
# This script fine-tunes MedSAM2 on Mpox lesion data
###############################################################################
...
# 4) Create scripts directory and copy fine-tuning script
mkdir -p ${SCRIPTS_DIR}
cd ${SCRIPTS_DIR}
...
cat > ${SCRIPTS_DIR}/run_medsam2_finetune.py << 'EOF'
#!/usr/bin/env python3
"""
...
if __name__ == "__main__":
    main()
EOF
chmod +x ${SCRIPTS_DIR}/run_medsam2_finetune.py

# 5) Create fine-tuning directory
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
FINETUNE_DIR=${FINETUNE_DIR}/${TIMESTAMP}
mkdir -p ${FINETUNE_DIR}

# 6) Check for preprocessed data for training
...
if [ ! -d "${NPY_DIR}" ] || [ -z "$(ls -A ${NPY_DIR} 2>/dev/null)" ]; then
    ...
    echo "Please run the data preparation script first with masks: sbatch hpc_medsam2_dataprep.sh"
    exit 1
fi

# 7) Run fine-tuning with MedSAM2
...
echo "Using NPY data directory: ${NPY_DIR}"
echo "Fine-tuning results will be saved to: ${FINETUNE_DIR}"
echo "=========================================================="

# Set batch size based on available GPU memory and model size
# Assuming 4 GPUs are available
NUM_GPUS=$(nvidia-smi --query-gpu=name --format=csv,noheader | wc -l)
if [ ${NUM_GPUS} -ge 4 ]; then
    BATCH_SIZE=16  # Larger batch size for multi-GPU setup
elif [ ${NUM_GPUS} -ge 1 ]; then
    BATCH_SIZE=8   # Smaller batch size for single GPU
else
    BATCH_SIZE=4   # Minimum batch size for CPU (not recommended)
    echo "WARNING: No GPUs found. Fine-tuning will be very slow on CPU."
fi

# Calculate appropriate learning rate based on batch size
# Rule of thumb: scale learning rate linearly with batch size
LEARNING_RATE=$(echo "scale=6; 1e-5 * ${BATCH_SIZE} / 8" | bc)

echo "Using batch size: ${BATCH_SIZE}"
echo "Using learning rate: ${LEARNING_RATE}"

# Run fine-tuning script
python ${SCRIPTS_DIR}/run_medsam2_finetune.py \
    --npy_dir ${NPY_DIR} \
...
if [ $? -eq 0 ]; then
    ...
    echo "Running inference with the fine-tuned model..."
    
    # Path to fine-tuned model
    FINETUNED_MODEL=${FINETUNE_DIR}/medsam2_mpox_finetuned.pth
    
    if [ -f "${FINETUNED_MODEL}" ]; then
        # Create inference results directory for fine-tuned model
        INFERENCE_RESULTS_DIR=${FINETUNE_DIR}/inference_results
        ...
        echo "Inference with fine-tuned model completed."
        echo "Results saved to: ${INFERENCE_RESULTS_DIR}"
    else
...
# 9) Create a symbolic link to the latest run for easy access
LATEST_LINK=${BASE_DIR}/medsam2_latest_finetune
rm -f ${LATEST_LINK} 2>/dev/null
ln -s ${FINETUNE_DIR} ${LATEST_LINK}

# 10) Print summary and next steps
echo "=========================================================="
echo "FINE-TUNING PIPELINE COMPLETED"
echo "=========================================================="
echo "Fine-tuning results directory: ${FINETUNE_DIR}"
echo "Symlink to latest run: ${LATEST_LINK}"
echo ""
echo "Next steps:"
echo "1. Review fine-tuning metrics and visualizations in ${FINETUNE_DIR}"
echo "2. Use the fine-tuned model for inference on new images:"
echo "   python run_medsam2_inference.py --npz_dir <new_data_dir> --sam2_checkpoint ${SAM2_CHECKPOINT} --medsam2_checkpoint ${FINETUNE_DIR}/medsam2_mpox_finetuned.pth"
echo "=========================================================="
